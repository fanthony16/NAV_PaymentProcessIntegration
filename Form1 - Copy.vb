...
Public Class Form1
	...
	Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
		...
		pv.UseDefaultCredentials = True
		pvc.UseDefaultCredentials = True
		bank.UseDefaultCredentials = True
		bnkL.UseDefaultCredentials = True

		'pr.Proxy = prxy
		'pr.Credentials = nc

		'Dim PRFilterArray As New List(Of PaymentVoucher.PaymentVoucherList_Filter)
		'Dim PRFilter As New PaymentVoucher.PaymentVoucherList_Filter
		...
		Dim pmv() As PaymentVoucher.PaymentVoucherList
		pmv = pv.ReadMultiple(PVFilterArray.ToArray, "", 100)
		...
		Do While i < pmv.Count
			...
			BVPayments.PaymentType = "A"
			BVPayments.ProcessingMode = "M"
			BVPayments.CustomerRef = pmr(0).No
			...
			BVPayments.Payee = pmr(0).PVLines(0).Payee
			...
			BVPayments.PayeeFaxNumber = ""
			BVPayments.PayeeBankCode = ""
			'''''''''''''''''''''''''''''''''Getting Bank Clearing Code''''''''''''''''''''


			BankListFilterCard.Field = BankList.BeneficiaryBankList_Fields.Bank_Code
			BankListFilterCard.Criteria = pmr(0).PVLines(0).Bank
			BankListFilterArrayCard.Add(BankListFilterCard)
			Dim PVBankList() As BankList.BeneficiaryBankList
			PVBankList = bnkL.ReadMultiple(BankListFilterArrayCard.ToArray, "", 100)
			...
			If IsNothing(pmr(0).PVLines(0).Payee_Account_No) = False Then
				BVPayments.PayeeAccount = pmr(0).PVLines(0).Payee_Account_No
			Else
				BVPayments.PayeeAccount = pmr(0).PVLines(0).Account_No
			End If

			If pmr(0).PVLines(0).Account_No.Length > 70 Then
				BVPayments.PaymentDetails1 = pmr(0).PVLines(0).Payment_Narration.Substring(0, 69).Replace(",", "")
				BVPayments.PaymentDetails2 = pmr(0).PVLines(0).Payment_Narration.Substring(70, pmr(0).PVLines(0).Payment_Narration.Length).Replace(",", "")
			Else
				BVPayments.PaymentDetails1 = pmr(0).PVLines(0).Payment_Narration.Replace(",", "")
				BVPayments.PaymentDetails2 = ""
			...
			BVPayments.PaymentCurrency = "NGN"
			''''''''''''''''''''''''''
			BVPayments.PaymentAmount = pmr(0).PVLines(0).Net_Amount
			BVPayments.PaymentPurpose = "PAYR"
			lstPayments.Add(BVPayments)
